package com.dsk.myexpense.expense_module.util.swipeLayout

import android.graphics.Canvas
import androidx.recyclerview.widget.RecyclerView

import com.dsk.myexpense.expense_module.util.swipeLayout.touchelper.ItemTouchHelper
    ...
    override fun onChildDraw(c: Canvas, recyclerView: RecyclerView,
        ...
        if (viewHolder is OnItemSwipePercentageListener) {
            (viewHolder as OnItemSwipePercentageListener).onItemSwipePercentage(
                    (-1f).coerceAtLeast(1f.coerceAtMost(dX / recyclerView.measuredWidth)).toDouble())
        }
